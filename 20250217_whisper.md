...
## WhisperKitを導入して驚異的な精度と速度を実現

そんな中、**暦本先生**が公開されているWhisper-cliを使った iPhoneでボタン長押し→音声入力→テキスト書き起こしがメモアプリに反映 できるスクリプトである[voice2memo](https://github.com/rkmt/voice2memo)が公開され、利用し始めました。これはiPhoneとMacを連携させることで、WhisperをiCloud環境で気軽に使える非常に素晴らしいスクリプトです。  
...
オリジナルのvoice2memoでも使われていますが、特に**Whisper の large-v3-turboモデル**を使用すると、ほとんど文字起こしのミスが発生せず、細かいディティールも正確に反映されます。これまで音声入力を使っていたときよりも、文章の質が格段に向上しました。
